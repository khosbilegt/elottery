...
function Months() {
  ...
  useEffect(() => {
    const cachedJson: { [key: string]: any } = JSON.parse(
      localStorage.getItem("elottery") || "{}"
    );
  ...
  useEffect(() => {
    if (months != null) {
      const updatedMonthNames = Object.keys(months).map(
  ...
  return (
    <div className="w-1/3 h-full text-center flex flex-col">
      ...
      <Modal
        ...
        footer={
          <Button className="btn primary" onClick={handleDatePickerOk}>
            Үүсгэх
          </Button>
        }
      >
